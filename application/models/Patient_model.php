...
class Patient_model extends CI_Model {
    ...
    public function getConsultant($patient_id, $opd_month) {
        ...
        $opd_query = $this->db->select('opd_details.appointment_date,opd_details.case_type,staff.name,staff.surname')
                ...
                ->where('opd_details.appointment_date <', date('Y-m-d',strtotime($last_date)))
                ->where('opd_details.patient_id', $patient_id)
                ->limit(1)
                ->get('opd_details');
    ...
    public function lastVisit($patient_id) {
        $query = $this->db->select('max(opd_details.appointment_date) as last_visit_date, max(opd_details.appointment_time) as last_visit_time')
                ->where('patient_id', $patient_id)
                ->get('opd_details');
        return $query->row_array();
    }
    ...
    public function getIpdDetails($id, $ipdid, $active = 'yes') {
        $this->db->select('patients.*,ipd_details.patient_id,ipd_details.date, ipd_details.time, ipd_details.case_type,ipd_details.ipd_no,ipd_details.id as ipdid,ipd_details.casualty,ipd_details.height,ipd_details.weight,ipd_details.bp,ipd_details.cons_doctor,ipd_details.refference,ipd_details.known_allergies,ipd_details.amount,ipd_details.credit_limit as ipdcredit_limit,ipd_details.symptoms,ipd_details.discharged as ipd_discharge,ipd_details.tax,ipd_details.bed,ipd_details.bed_group_id,ipd_details.note as ipdnote,ipd_details.bed,ipd_details.bed_group_id,ipd_details.payment_mode,ipd_billing.status,ipd_billing.gross_total,ipd_billing.discount,ipd_billing.date as discharge_date,ipd_billing.tax,ipd_billing.net_amount,ipd_billing.total_amount,ipd_billing.other_charge,ipd_billing.generated_by,ipd_billing.id as bill_id,staff.id as staff_id,staff.name,staff.surname,organisation.organisation_name,bed.name as bed_name,bed.id as bed_id,bed_group.name as bedgroup_name,floor.name as floor_name,agent_details.agent_id,agent_details.name as agent_name')->from('patients');
        $this->db->join('ipd_details', 'patients.id = ipd_details.patient_id', "left");
        $this->db->join('agent_details', 'agent_details.id = patients.agent_details', "left");
    ...
    public function getOPDetails($id, $opdid = null) {
        ...
        $this->db->select('opd_details.*,patients.organisation,patients.old_patient,staff.id as staff_id,staff.name,staff.surname,consult_charges.standard_charge')->from('opd_details'); ///,opd_patient_charges.apply_charge
        $this->db->join('staff', 'staff.id = opd_details.cons_doctor', "inner");
    ...
    public function getCharges($patient_id, $ipdid = '') {
        $query = $this->db->select("sum(apply_charge * unit) as charge")->where("patient_id", $patient_id)->where("ipd_id", $ipdid)->get("patient_charges");
        return $query->row_array();
    }
