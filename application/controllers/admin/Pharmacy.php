...
class Pharmacy extends Admin_Controller
{
    ...
    public function add()
    {
        ...
        $this->form_validation->set_rules(
            ...
            'medicine_name', $this->lang->line('medicine') . " " . $this->lang->line('category'), array('required', array('check_exists', array($this->medicine_category_model, 'valid_medicine_name'))
    ...
    public function addBill()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $amt = $this->input->post('paid_amount');
            ...
            $data = array(
                ...
                'due_amt' => $this->input->post('due_amount'),
            ...
            if ($bill_id) {
                ...
                /*===============================Agent Commission Section Start =================================*/



                $p = $this->patient_model->patientDetails($patient_id);

                $a = $this->agent_model->get_agent($p['agent_details']);

                $c = $this->agent_model->get_cat($a['agent_cat']);

                $pay = $amt * $c['commission'] / 100;

                //echo $amt,$pay,$c['commission'];die; 
                ...
                $up = array(
                    ...
                    'patient_id' => $p['patient_unique_id'],
                ...
                if (!empty($p['agent_details'])) {

                    $this->agent_model->insert_pay($up);

                }



                /*===============================Agent Commission Section End =================================*/
                ...
                foreach ($medicine_name as $key => $value) {
                    ...
                    $cat = $this->db->select('medicine_category_id')->where('id', $medicine_name[$i])->get('pharmacy')->row_array();

                    $detail = array(

                        'pharmacy_bill_basic_id' => $bill_id,

                        'medicine_category_id' => empty($medicine_category_id[$i]) ? $cat['medicine_category_id'] : $medicine_category_id[$i],
