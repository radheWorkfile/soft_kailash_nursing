...
class patient extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $dates = date('Y-m-d', strtotime($appointment_date));
            $times = date('h:i:s', strtotime($appointment_date)); 
            ...
            $opd_data = array(
                ...
                // 'appointment_date' => strftime('%Y-%m-%dT%H:%M:%S', strtotime($appointment_date)),
    ...
    public function search()
    {
        ...
        foreach ($resultlist as $visits) {
            ...
            // israel chnges for errro land_db.php not found ====satrt==========

            $consultant = $this->patient_model->getConsultant($patient_id, $opd_month);
            
           // israel chnges for errro land_db.php not found ====end==========

            $resultlist[$i]["total_visit"] = $total_visit["total_visit"];

            $resultlist[$i]["last_visit_date"] = $last_visit["last_visit_date"];
            $resultlist[$i]["last_visit_time"] = $last_visit["last_visit_time"];
    ...
    public function ipdprofile($id, $ipdid = '', $active = 'yes')
    {
        ...
        if (!empty($id)) {
            ...
            $data['charge_cat'] = $this->db->get("charge_categories")->result_array();
    ...
    public function add_diagnosis()
    {
        ...
        $this->form_validation->set_rules('diagnosis_center_name', "Diagnosis Center Name", 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $msg = array(
                ...
                'diagnosis_center_name' => form_error('diagnosis_center_name'),
            ...
            $data = array(
                ...
                'diagnosis_center_name' => $this->input->post('diagnosis_center_name'),
    ...
    public function add_inpatient()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $ipd_data = array(
                ...
                // 'date' => strftime('%Y-%m-%dT%H:%M:%S', strtotime($appointment_date)),
                'date'   => $dates,
                'time'   => $times,
    ...
    public function ipdCharge()
    {
        ...
        $org_id = ($this->input->post('organisation_id') == 'blank') ? '' : $this->input->post('organisation_id');
    ...
    public function dischargepatientReport()
    {
        ...
        $select = 'ipd_details.*,payment.paid_amount, payment.date as payment_date, 

                    staff.name,staff.surname,patients.id as pid,patients.patient_name,patients.patient_unique_id,patients.guardian_name,patients.address,patients.admission_date,patients.gender,patients.mobileno,patients.age';
    ...
    public function discharge_cerificate()
    {
        ...
        $data['diagnosis'] = $this->patient_model->getDiagnosisDetails($id);
        ...
        $data['ipd'] = $this->patient_model->getIpdDetails($id, $ipdid, $active);

        $data['bill_info'] = $this->patient_model->getBillInfo($id);

        $data['patient'] = $this->patient_model->patientDetails($id);

        $data['doctor'] = $this->patient_model->getPatientConsultant($id, $ipdid);

        $data['hospital'] = $this->setting_model->get();

        $this->load->view('admin/printing/discharge_certificate', $data);

    }



    public function ipdprint()
    {
        ...
        $data['result'] = $this->patient_model->getIpdDetails($id, $ipdid);

        $print_details = $this->printing_model->get('', 'ipd');
        ...
        $this->load->view('admin/patient/ipdprint', $data);
