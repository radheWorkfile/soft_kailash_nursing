...
class Admin extends Admin_Controller
{
    ...
    public function dashboard()
    {
        ...
        $stock = $this->pharmacy_model->searchFullText();
        $avl_stock_price=0;
       foreach($stock as $st)
       {
        $price=$st['mrp']*$st['total_qty'];

          $avl_stock_price  = $avl_stock_price +  $price;

       }
       $data['avl_stock_price'] = $avl_stock_price;
     


        $data['jsonarr'] = $jsonarr;

        $data['resultlist'] = $this->db->select('*')->order_by('date', 'desc')->limit('10')->from('appointment')->get()->result_array();

        $data['doc'] = $this->staff_model->getEmployeeByRoleID(3);
